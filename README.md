...
- About GANs:
  ...
  - A review of the training instability issue of GANs
    ...
    - LGANs extended: 
      ...
      - We believe gradient penalty / restriction / regularization based methods all have similar effects, based on our analysis upon how Lipschitz regularization works. But we have to highlight that the Lipschitz regularization is not the sole solution to the training instability issue of GANs. [Coulomb GANs](https://arxiv.org/abs/1708.08819) as another instance: it makes use of electronic properties between positive and negative electrons, instead of minimizing divergence / distance. 
    ...
    - How unregularized GANs works:
      ...
      - The gradient uninformativeness issue designated in LGANs is actually a small part (while the most important part) of gradient issues in unregularized GANs. It also involves gradient vanishing (which actually has two types; original GANs suffers from Type-I; Least-Square-GANs, avoiding Type-I, but still suffers from Type-II). Other gradient issues in unregularized GANs include theoretically undefined gradients and non-convergent-state of Type-II gradient vanishing, and surely the previously mentioned local-greedy property. See Appendix B in [2019.02 my arXiv report](https://arxiv.org/abs/1902.05687v2). 
