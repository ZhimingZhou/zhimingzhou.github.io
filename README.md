...
- Actually, all previous descriptions are based on an assumption, i.e., the so-called Lipschitz is with respect to the Euclidean distance / norm. When switching to other norms the property will get changed. See Appendix A.4 in [2019.02 my arXiv report](https://arxiv.org/abs/1902.05687v2) for some basic arguments on this. The conclusion is that the property that gradients directly point towards real samples only holds when it is Euclidean distance / norm. Different norms will induce different gradients with different properties. [Banach WGANs](https://arxiv.org/abs/1806.06621) might also help your understanding. 
